...
description: "SpecDrive 预设路径：Bug fix / 热修复。跳过 brainstorming，直接 open → build → verify → archive。适用于行为修复、不涉及新 capability 设计的场景。"
---

# SpecDrive 预设路径：Hotfix
...
**不适用**：如修复过程发现需要架构调整，应升级为完整 `/specdrive` 流程。
...
复用 SpecDrive open 能力创建 change，但使用 hotfix 默认值：不执行 `openspec-explore` 长探索，直接进入精简 change 创建。
...
使用 hotfix 默认值：`build_mode: direct`。跳过 Superpowers `brainstorming` 和 `writing-plans`（除非任务 > 3 个；若超过 3 个任务，转入 `/specdrive-build` 的计划与执行方式选择——注意这不触发 full workflow 升级，仅切换执行方式）。

继续或开始修改前，按 `specdrive/reference/dirty-worktree.md` 协议处理未提交改动。若归因后发现修复范围超出 hotfix，按本文件“升级条件”处理。
...
复用 `/specdrive-verify`，由 specdrive-verify 的规模评估决定轻量或完整验证。

**立即执行：** 使用 Skill 工具加载 `specdrive-verify` 技能。禁止跳过此步骤。

无 delta spec 的小范围 hotfix 通常满足轻量验证条件（≤ 3 tasks、≤ 2 files），specdrive-verify 的规模评估会选择轻量验证路径（5 项快速检查）。若 hotfix 创建了 delta spec，则根据 specdrive-verify 的规模评估规则进入完整验证路径。

验证通过后，按 `/specdrive-verify` 的规则将 `.specdrive.yaml` 的 `verify_result` 记录为 `pass`，归档前不得跳过该状态。验证通过后仍必须进入 `/specdrive-archive` 的归档前最终确认，不得自动运行归档脚本。

### 5. 归档（preset archive）

复用 `/specdrive-archive`。归档前必须满足 `.specdrive.yaml` 中 `verify_result: pass`，并等待 `/specdrive-archive` 的归档前最终确认。

**立即执行：** 使用 Skill 工具加载 `specdrive-archive` 技能进行归档。禁止跳过此步骤。
如有 delta spec，按 specdrive-archive 规则同步到 main spec，并处理关联 Design Doc 与 Plan 的归档标注。
...
<IMPORTANT>
Hotfix 流程默认 **一次性连续执行**。调用 `/specdrive-hotfix` 后，agent 在 hotfix 自有步骤间自动推进，不主动停顿。**例外**：若 `auto_transition: false`，则在每个 phase 边界（build/verify/archive 之间）停下，由用户手动运行下一阶段命令——此时连续执行降级为逐阶段手动推进，详见下方「自动衔接下一阶段」。但无论 `auto_transition` 取何值，以下情况都必须暂停等待用户确认：

1. 遇到升级条件（见"升级条件"章节），**必须使用当前平台可用的用户输入/确认机制暂停并等待用户明确确认**升级为完整流程
2. 任务超过 3 个转入 `/specdrive-build` 时的工作区隔离和执行方式选择
3. 验证阶段（specdrive-verify）的验证失败决策和分支处理决策
4. 归档前最终确认（specdrive-archive 执行归档脚本前）
...
满足以下**任一**条件时，停止 hotfix 流程，升级为完整 `/specdrive`：
...
满足升级条件时**必须使用当前平台可用的用户输入/确认机制暂停并等待用户明确确认**升级为完整 `/specdrive` 流程。不得直接进入 `/specdrive-design`，不得自动补充 Design Doc。若当前平台没有结构化提问工具，则在对话中提出升级确认问题并停止流程，等待用户回复后才能继续。
...
然后在当前 change 基础上补充 Design Doc：**立即使用 Skill 工具加载 `specdrive-design` skill**，后续正常走完整流程。若用户不确认升级，停止 hotfix 并报告当前变更已超出 hotfix 适用范围。
...
- **阶段守卫**：build → verify 前运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> build --apply`，verify → archive 前按 `/specdrive-verify` 规则运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> verify --apply`

## 自动衔接下一阶段

> **术语区分**：阶段守卫 `--apply` 推进 `.specdrive.yaml` 的 `phase` 字段——这一步**始终发生**，与 `auto_transition` 无关。本节的「自动衔接」只决定**是否自动调用下一个 skill**。
...
- `NEXT: auto` → 调用 `SKILL` 指向的 skill 继续 hotfix 流程（`phase: build` 返回 `specdrive-hotfix`，`verify` 返回 `specdrive-verify`，`archive` 返回 `specdrive-archive`）
