---
name: specdrive-verify
description: "SpecDrive 阶段 4：验证与收尾。用 /specdrive-verify 调用。验证实现符合设计，处理开发分支。"
---

# SpecDrive 阶段 4：验证与收尾（Verify）
...
. "$SPECDRIVE_ENV"
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" check <change-name> verify
```
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" scale <change-name>
```

脚本自动统计任务数、增量规格数、变更文件数，判断使用 light 或 full 验证模式，并设置 verify_mode 字段。判定规则（满足任一即 full）：任务数 > 3、delta spec 能力数 > 1、变更文件数 > 4。

验证开始前，按 `specdrive/reference/dirty-worktree.md` 协议检查并处理未提交改动。verify 阶段的特殊处理：
...
注意：verify-fail 回退到 build 时 `branch_status` 不会被重置。如果首次 verify 已完成分支处理，修复后再次进入 verify 时跳过已完成的分支处理步骤，直接使用 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <change-name> branch_status handled` 保留原有分支处理结果。

注意：如果 build 阶段每个任务都已提交，脚本基于工作区 diff 的文件数可能低估改动规模。此时必须读取 plan 文件头的 `base-ref` 并用提交区间复核：

```bash
PLAN=$("$SPECDRIVE_BASH" "$SPECDRIVE_STATE" get <change-name> plan)
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <change-name> verify_mode full
```

**覆盖机制**：如 agent 或用户认为自动评估结果不合适，可随时通过 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <change-name> verify_mode <light|full>` 手动覆盖。

### 1b. 验证失败决策（阻塞点）

验证不通过时**必须使用当前平台可用的用户输入/确认机制暂停并等待用户决定修复或接受偏差**。不得自动运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" transition <change-name> verify-fail`，也不得自动调用 `/specdrive-build`。若当前平台没有结构化提问工具，则在对话中提出修复/接受偏差选项并停止流程，等待用户回复后才能继续。
...
用户选择后按以下方式继续：
- **全部修复**：运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" transition <change-name> verify-fail`，然后调用 `/specdrive-build` 修复
...
```bash
RECORDED_HASH=$("$SPECDRIVE_BASH" "$SPECDRIVE_STATE" get <change-name> handoff_hash)
CURRENT_HASH=$("$SPECDRIVE_BASH" "$SPECDRIVE_HANDOFF" <change-name> --hash-only 2>/dev/null || echo "")
```
...
**不通过时**：报告失败项，进入 Step 1b 的验证失败决策阻塞点。用户选择修复后，才执行以下命令记录失败并回退到 build 阶段，然后调用 `/specdrive-build` 修复：
...
验证不通过时：报告缺失项，进入 Step 1b 的验证失败决策阻塞点。用户选择修复后，才执行以下命令记录失败并回退到 build 阶段，然后调用 `/specdrive-build` 补充：
...
- 若检查项 6 发现矛盾（delta spec 有内容但 design doc 未体现），**必须使用当前平台可用的用户输入/确认机制以单选题形式暂停并等待用户选择处理方式**，不得自动选择。选项：
  ...
  - 选项 B：用户选择 B 后，运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" transition <change-name> verify-fail`，然后调用 `/specdrive-build`；由 `/specdrive-build` 的 Spec 增量更新规则加载 Superpowers `brainstorming` 更新 Design Doc + delta spec
...
验证报告必须落盘，并在 `.specdrive.yaml` 中记录；分支处理完成后也必须写入状态字段。不要手动设置 `verify_result: pass`，由阶段守卫 `--apply` 推进。
...
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <change-name> verification_report docs/superpowers/reports/YYYY-MM-DD-<change-name>-verify.md
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <change-name> branch_status handled
```
...
- 验证报告通过
- 分支已处理
- `.specdrive.yaml` 中 `verification_report` 指向已存在的验证报告文件
- `.specdrive.yaml` 中 `branch_status: handled`
- **阶段守卫**：运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> verify --apply`，全部 PASS 后由守卫通过 `specdrive-state transition verify-pass` 推进到 `phase: archive`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）

验证和分支处理均完成后，运行阶段守卫推进 phase（此步骤与 `auto_transition` 无关）：

```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> verify --apply
```
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" check <change-name> verify --recover
```
...
注意：无论 `NEXT` 为 `auto` 还是 `manual`，`specdrive-archive` 进入后必须先执行归档前最终确认阻塞点，等待用户明确选择「确认归档」后才允许运行归档脚本。不得因为验证已通过就自动归档。
