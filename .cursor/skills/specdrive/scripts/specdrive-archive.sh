#!/bin/bash
# SpecDrive Archive — automates the archive phase in one command
# Usage: specdrive-archive.sh <change-name> [--dry-run]
# Exit 0 = archive complete, exit 1 = fatal error

set -euo pipefail

SPECDRIVE_BASH="${SPECDRIVE_BASH:-${BASH:-bash}}"
SPECDRIVE_OPENSPEC="${SPECDRIVE_OPENSPEC:-openspec}"
...
YAML="$CHANGE_DIR/.specdrive.yaml"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd -P)"
STATE_SH="$SCRIPT_DIR/specdrive-state.sh"
TODAY=$(date -u +%Y-%m-%d)
...
echo "=== SpecDrive Archive: $CHANGE ===" >&2

# --- Step 1: Read .specdrive.yaml, extract paths ---

yaml_field() {
  local field="$1"
  if [ -f "$STATE_SH" ]; then
    "$SPECDRIVE_BASH" "$STATE_SH" get "$CHANGE" "$field" 2>/dev/null
  else
    if [ -f "$YAML" ]; then
      local value
...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  if ! command -v "$SPECDRIVE_OPENSPEC" >/dev/null 2>&1; then
    red "FATAL: OpenSpec CLI not found: $SPECDRIVE_OPENSPEC"
    red "Install OpenSpec or set SPECDRIVE_OPENSPEC to the openspec executable."
    exit 1
  fi

  "$SPECDRIVE_OPENSPEC" archive "$CHANGE" --yes >&2
...
# --- Step 7: Mark archived via specdrive-state transition ---

ARCHIVE_YAML="$ARCHIVE_DIR/.specdrive.yaml"

if [ "$DRY_RUN" -eq 1 ]; then
  step_dry_run "Would set archived: true in $ARCHIVE_YAML"
else
  if [ -f "$ARCHIVE_YAML" ]; then
    "$SPECDRIVE_BASH" "$STATE_SH" transition "$ARCHIVE_NAME" archived >/dev/null
    step_ok "archived: true"
  else
    step_fail "archived: true (.specdrive.yaml not found after move)"
  fi
fi
