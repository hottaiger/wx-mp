#!/bin/bash
# specdrive-hook-guard.sh — PreToolUse hook for SpecDrive phase enforcement
#
# Blocks file writes (Write/Edit) when the active SpecDrive change is in
...
if [ -d "openspec/changes" ]; then
  for dir in openspec/changes/*/; do
    ...
    if [ -f "${dir}.specdrive.yaml" ]; then
      YAML_FILE="${dir}.specdrive.yaml"
      break
    fi
  done
fi
  ...
  open|design|archive)
    ...
    case "$PHASE" in
      ...
      design)
        ...
        echo "  💡 完成 Design Doc 后运行 specdrive-guard design --apply 进入 build" >&2
        ;;
      archive)
