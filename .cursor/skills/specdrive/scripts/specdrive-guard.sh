#!/bin/bash
# SpecDrive Phase Guard — validates exit conditions before phase transitions
# Usage: specdrive-guard.sh <change-name> <current-phase> [--apply]
...
SPECDRIVE_BASH="${SPECDRIVE_BASH:-${BASH:-bash}}"
...
if [ "${SPECDRIVE_GUARD_SOURCE_ONLY:-0}" = "1" ]; then
  CHANGE="${CHANGE:-}"
  PHASE="${PHASE:-}"
  APPLY="${APPLY:-0}"
...
plan_tasks_all_done() {
  ...
  if [ ! -f "$plan" ]; then
    ...
    echo "Next: restore the Superpowers plan file or update .specdrive.yaml plan before leaving build." >&2
    return 1
  fi
...
yaml_field_value() {
  local field="$1"
  local yaml="$CHANGE_DIR/.specdrive.yaml"
  if [ -f "$yaml" ]; then
    local value
...
project_config_value() {
  ...
  for config in ".specdrive.yaml" "comet.yaml" ".comet.yml" "comet.yml"; do
    if [ -f "$config" ]; then
...
preflight() {
  ...
  if [ ! -f "$CHANGE_DIR/.specdrive.yaml" ]; then
    red "FATAL: .specdrive.yaml not found in $CHANGE_DIR"
    exit 1
  fi

  # Schema validation
  local validate_script
  validate_script="$SCRIPT_DIR/specdrive-yaml-validate.sh"
  if [ -f "$validate_script" ]; then
    if ! "$SPECDRIVE_BASH" "$validate_script" "$CHANGE" 2>/dev/null; then
      "$SPECDRIVE_BASH" "$validate_script" "$CHANGE" || true
      red "FATAL: .specdrive.yaml schema validation failed"
      exit 1
    fi
  fi
}
...
build_passes() {
  if [ "${SPECDRIVE_SKIP_BUILD:-0}" = "1" ]; then
    return 0
  fi
...
isolation_selected() {
  ...
  case "$isolation" in
    ...
    *)
      ...
      echo "  \"\$SPECDRIVE_BASH\" \"\$SPECDRIVE_STATE\" set $CHANGE isolation <branch|worktree>" >&2
      return 1
      ;;
  esac
}
...
build_mode_selected() {
  ...
  case "$build_mode" in
    ...
    *)
      ...
      echo "  \"\$SPECDRIVE_BASH\" \"\$SPECDRIVE_STATE\" set $CHANGE build_mode <subagent-driven-development|executing-plans>" >&2
      return 1
      ;;
  esac
}
...
subagent_dispatch_confirmed() {
  ...
  echo "  \"\$SPECDRIVE_BASH\" \"\$SPECDRIVE_STATE\" set $CHANGE subagent_dispatch confirmed" >&2
  echo "Or ask the user to switch to executing-plans and run:" >&2
  echo "  \"\$SPECDRIVE_BASH\" \"\$SPECDRIVE_STATE\" set $CHANGE build_mode executing-plans" >&2
  return 1
}
...
tdd_mode_selected() {
  ...
  case "$tdd_mode" in
    ...
    *)
      ...
      echo "  \"\$SPECDRIVE_BASH\" \"\$SPECDRIVE_STATE\" set $CHANGE tdd_mode <tdd|direct>" >&2
      return 1
      ;;
  esac
}
...
design_handoff_context_valid() {
  ...
  if [ -z "$context" ] || [ "$context" = "null" ]; then
    echo "handoff_context is missing from .specdrive.yaml" >&2
    echo "Next: run \"\$SPECDRIVE_BASH\" \"\$SPECDRIVE_HANDOFF\" $CHANGE design --write before invoking Superpowers." >&2
    return 1
  fi
  if [ ! -s "$context" ]; then
    echo "handoff_context does not point to a non-empty file: $context" >&2
    echo "Next: regenerate the design handoff with specdrive-handoff.sh." >&2
    return 1
  fi
  ...
  if [ "$actual_hash" != "$recorded_hash" ]; then
    ...
    echo "Next: rerun specdrive-handoff.sh so Superpowers receives the current OpenSpec context." >&2
    return 1
  fi
...
design_handoff_markdown_traceable() {
  ...
  grep -q '^Generated-by: specdrive-handoff\.sh$' "$markdown" || {
...
guard_design() {
  ...
  if [ -n "$design_doc" ] && [ "$design_doc" != "null" ]; then
    ...
    warn "  [WARN] No design_doc recorded in .specdrive.yaml (optional for hotfix/tweak)"
  fi
}
...
design_doc_recorded() {
  ...
  echo "Next: create the Design Doc and run: \"\$SPECDRIVE_BASH\" \"\$SPECDRIVE_STATE\" set $CHANGE design_doc <path>" >&2
  return 1
}
...
apply_state_update() {
  local state_sh="$SCRIPT_DIR/specdrive-state.sh"
  local p="$1"

  if [ -f "$state_sh" ]; then
    case "$p" in
      open)   "$SPECDRIVE_BASH" "$state_sh" transition "$CHANGE" open-complete ;;
      design) "$SPECDRIVE_BASH" "$state_sh" transition "$CHANGE" design-complete ;;
      build)  "$SPECDRIVE_BASH" "$state_sh" transition "$CHANGE" build-complete ;;
      verify) "$SPECDRIVE_BASH" "$state_sh" transition "$CHANGE" verify-pass ;;
    esac
  else
    red "FATAL: specdrive-state.sh not found; cannot apply state transition"
    exit 1
  fi
}
...
if [ "${SPECDRIVE_GUARD_SOURCE_ONLY:-0}" = "1" ]; then
  ...
  red "ERROR: SPECDRIVE_GUARD_SOURCE_ONLY=1 is only for sourcing, not direct execution" >&2
  ...
  if [ "$APPLY" -eq 1 ]; then
    ...
    case "$PHASE" in
      open)
        ...
        green "  [APPLY] .specdrive.yaml updated: phase=$new_phase"
        ;;
      design) green "  [APPLY] .specdrive.yaml updated: phase=build" ;;
      build)  green "  [APPLY] .specdrive.yaml updated: phase=verify, verify_result=pending" ;;
      verify) green "  [APPLY] .specdrive.yaml updated: phase=archive, verify_result=pass" ;;
    esac
  fi
  exit 0
fi
