#!/bin/bash
# SpecDrive Handoff — creates machine-owned context packages between phases
# Usage: specdrive-handoff.sh <change-name> design --write [--full]
#        specdrive-handoff.sh <change-name> --hash-only
...
write_markdown_context() {
  local output="$1"
  {
    ...
    echo ""
    echo "Generated-by: specdrive-handoff.sh"
    echo ""
...
write_json_context() {
  local output="$1"
  {
    ...
    echo "  \"generated_by\": \"specdrive-handoff.sh\","
...
if [ "$PHASE" != "design" ] || [ "$MODE" != "--write" ]; then
  red "Usage: specdrive-handoff.sh <change-name> design --write [--full]"
  exit 1
fi
case "$FULL_FLAG" in
...
if [ ! -f "$YAML" ]; then
  red "ERROR: .specdrive.yaml not found at $YAML"
  exit 1
fi
...
HANDOFF_DIR="$CHANGE_DIR/.specdrive/handoff"
...
if [ -x "$STATE_SH" ] || [ -f "$STATE_SH" ]; then
  "$SPECDRIVE_BASH" "$STATE_SH" set "$CHANGE" handoff_context "$CONTEXT_JSON" >/dev/null
  "$SPECDRIVE_BASH" "$STATE_SH" set "$CHANGE" handoff_hash "$CONTEXT_HASH" >/dev/null
else
  red "ERROR: specdrive-state.sh not found; cannot record handoff fields"
  exit 1
fi
