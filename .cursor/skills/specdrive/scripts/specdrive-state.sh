#!/bin/bash
# SpecDrive State — unified interface for .specdrive.yaml state management
# Usage: specdrive-state.sh <subcommand> <change-name> [args...]
#
# Subcommands:
#   init <change-name> <workflow>  — Initialize .specdrive.yaml with workflow defaults
#   get <change-name> <field>       — Read a field value from .specdrive.yaml
...
yaml_file_for() {
  ...
  echo "$change_dir/.specdrive.yaml"
}

project_context_compression() {
  local value="off"
  local source="default"
  if [ -n "${SPECDRIVE_CONTEXT_COMPRESSION:-}" ]; then
    value="$SPECDRIVE_CONTEXT_COMPRESSION"
    source="SPECDRIVE_CONTEXT_COMPRESSION"
  elif [ -f ".specdrive/config.yaml" ]; then
    value=$(yaml_field "context_compression" ".specdrive/config.yaml")
    value="${value:-off}"
    source=".specdrive/config.yaml"
  fi
...
project_auto_transition_default() {
  local value="true"
  local source="default"
  if [ -n "${SPECDRIVE_AUTO_TRANSITION:-}" ]; then
    value="$SPECDRIVE_AUTO_TRANSITION"
    source="SPECDRIVE_AUTO_TRANSITION"
  elif [ -f ".specdrive/config.yaml" ]; then
    local raw
    raw=$(yaml_field "auto_transition" ".specdrive/config.yaml" 2>/dev/null || true)
    if [ -n "$raw" ]; then
      value="$raw"
      source=".specdrive/config.yaml"
    fi
...
cmd_init() {
  ...
  # Check if .specdrive.yaml already exists
  if [ -f "$yaml_file" ]; then
    red "ERROR: .specdrive.yaml already exists at $yaml_file"
    exit 1
  fi
...
cmd_get() {
  ...
  # Check if .specdrive.yaml exists
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .specdrive.yaml not found at $yaml_file"
    exit 1
  fi
...
cmd_set() {
  ...
  case "$field" in
    phase)
      ...
      yellow "  Consider using: specdrive-state.sh transition <change-name> <event>" >&2
      ;;
...
cmd_check() {
  ...
  local yaml_file="$change_dir/.specdrive.yaml"
  ...
  case "$phase" in
    open)
      check_pass ".specdrive.yaml exists"
...
cmd_recover() {
  ...
  case "$phase" in
    ...
    archive)
      ...
      echo ""
      echo "Recovery action: Run /specdrive-archive to complete archiving."
      ;;
    *)
...
cmd_scale() {
  ...
  # 3. Changed files: prefer plan base-ref, then .specdrive.yaml base_ref, fall back to worktree diff
  ...
  if git rev-parse --git-dir > /dev/null 2>&1; then
    ...
    # Fallback to base_ref stored in .specdrive.yaml (set during init)
  ...
  # Update verify_mode in .specdrive.yaml
...
cmd_next() {
  ...
  local yaml_file="$change_dir/.specdrive.yaml"
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .specdrive.yaml not found at $yaml_file" >&2
    exit 1
  fi
...
case "$SUBCOMMAND" in
  init)
    if [ $# -lt 2 ]; then
      red "Usage: specdrive-state.sh init <change-name> <workflow>" >&2
  ...
  get)
    if [ $# -lt 2 ]; then
      red "Usage: specdrive-state.sh get <change-name> <field>" >&2
      exit 1
    fi
    cmd_get "$@"
    ;;
  set)
    if [ $# -lt 3 ]; then
      red "Usage: specdrive-state.sh set <change-name> <field> <value>" >&2
      exit 1
    fi
    cmd_set "$@"
    ;;
  transition)
    if [ $# -lt 2 ]; then
      red "Usage: specdrive-state.sh transition <change-name> <event>" >&2
  ...
  check)
    if [ $# -lt 2 ]; then
      red "Usage: specdrive-state.sh check <change-name> <phase> [--recover]" >&2
  ...
  scale)
    if [ $# -lt 1 ]; then
      red "Usage: specdrive-state.sh scale <change-name>" >&2
      exit 1
    fi
    cmd_scale "$@"
    ;;
  next)
    if [ $# -lt 1 ]; then
      red "Usage: specdrive-state.sh next <change-name>" >&2
      exit 1
    fi
    cmd_next "$@"
    ;;
  *)
    ...
    echo "" >&2
    echo "Usage: specdrive-state.sh <subcommand> <change-name> [args...]" >&2
    echo "" >&2
    echo "Subcommands:" >&2
    echo "  init <change-name> <workflow>  — Initialize .specdrive.yaml with workflow defaults" >&2
    echo "  get <change-name> <field>       — Read a field value from .specdrive.yaml" >&2
    echo "  set <change-name> <field> <val> — Update a field value in .specdrive.yaml" >&2
