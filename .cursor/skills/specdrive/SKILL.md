---
name: specdrive
description: "SpecDrive — OpenSpec + Superpowers 双星开发流程。用 /specdrive 启动，自动检测阶段并分发到子命令。五阶段：开启 → 深度设计 → 计划与构建 → 验证与收尾 → 归档。"
---

# SpecDrive — OpenSpec + Superpowers 双星开发流程
...
**Preset 检测优先级最高**：
- 用户明确描述为 bug fix / 热修复 + 满足 hotfix 条件 → 直接 `/specdrive-hotfix`
- 用户明确描述为文案/配置/文档/prompt 小调整 + 满足 tweak 条件 → 直接 `/specdrive-tweak`
- 未命中 preset → 按下表处理

| 活跃 change | 用户输入 | 行为 |
|-------------|---------|------|
| 无 | 非 preset 输入 | → 调用 `/specdrive-open` |
| 恰好 1 个 | `/specdrive <描述>` | → **询问**：继续该变更 or 创建新变更 |
| 多个 | `/specdrive <描述>` | → **询问**：继续现有变更 or 创建新变更；若选继续 → 列出清单让用户选择 |
| 恰好 1 个 | `/specdrive`（无描述） | → 自动选中，进入 Step 1 |
| 多个 | `/specdrive`（无描述） | → 列出清单让用户选择 |

<IMPORTANT>
当用户选择「创建新变更」时，**必须调用 `/specdrive-open`**（禁止直接调用 `/opsx:new`）。
`/specdrive-open` 负责完整双初始化：OpenSpec artifacts（由内部 `/opsx:new` 创建）+ `.specdrive.yaml` 状态文件。
直接调用 `/opsx:new` 会缺失 `.specdrive.yaml`，导致后续阶段判定失败。
</IMPORTANT>

**Step 1: 读取 `.specdrive.yaml` 状态元数据**

优先读取 `openspec/changes/<name>/.specdrive.yaml`。不存在时回退到 `openspec status --change "<name>" --json`、`tasks.md` 和 `docs/superpowers/` 文件检查。

**断点恢复规则**：
- 每次恢复上下文时，先重新执行 Step 0 和 Step 1，不依赖对话历史判断阶段
- 只要存在 active change 且工作区有未提交改动，必须按 `specdrive/reference/dirty-worktree.md` 协议处理。该协议定义了检查步骤、归因分类和禁令，本文件不重复
- 若 `phase: build`，先检查 `build_pause`、`plan`、`build_mode` 和 `isolation`（详见下方）：
  - 若 `build_pause: plan-ready` 但 `isolation` 和 `build_mode` 已经设置，则视为 stale pause：先输出 `[SPECDRIVE] 检测到 stale pause（build_pause=plan-ready 但 isolation/build_mode 已设置），自动清除并继续`，再运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> build_pause null`，然后读取 tasks.md 的下一个未勾选任务并按 `build_mode` 恢复执行
  - 若 `build_pause: plan-ready` 且 plan 文件存在，但 `isolation` 或 `build_mode` 尚未设置，回到 `/specdrive-build` 的 plan-ready 恢复点，提示用户继续选择隔离方式和执行方式，不重新生成 plan
  - 若 `build_pause: plan-ready` 但 plan 文件缺失，回到 `/specdrive-build` 处理状态损坏或重新生成 plan
  - 若 `build_mode`、`isolation` 或 `tdd_mode` 未设置，回到 `/specdrive-build` 对应步骤补充后再执行
  - 若均已设置，读取 tasks.md 的下一个未勾选任务，并按 `build_mode` 恢复执行：
    - 若 `build_mode: subagent-driven-development`，不得在主窗口直接执行任务；必须回到 `/specdrive-build` 的后台 subagent 调度规则，由主窗口只做协调
    - 其他执行方式按 `/specdrive-build` 的对应规则继续
- 若 `phase: verify` 且 `verify_result: fail`，进入验证失败决策阻塞点：暂停并询问用户修复或接受偏差；用户选择修复后才运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" transition <name> verify-fail` 并调用 `/specdrive-build`
- 若 `phase: open` 但 proposal/design/tasks 已完整，先运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> open --apply` 修正状态，再继续判定
- 若 `phase: archive`，只允许调用 `/specdrive-archive`；`/specdrive-archive` 必须先等待归档前最终确认，归档成功后 change 会移动到 archive 目录，不再对原活跃目录运行 guard

**Step 2: 阶段判定**（按顺序，命中即停）

1. `archived: true` 或 change 已移入 archive → 流程已完成
2. `verify_result: pass` 且 `archived` 不是 `true` → `/specdrive-archive`（先进行归档前最终确认）
3. `verify_result: fail` → 进入验证失败决策阻塞点（暂停询问修复或接受偏差；用户选择修复后才 `verify-fail` 并 `/specdrive-build`）
4. `phase: verify` 或 tasks.md 全部勾选 → `/specdrive-verify`
5. `phase: build` 或已有 Design Doc 但计划/执行未完成 → 优先按 workflow 路由：`hotfix` → `/specdrive-hotfix`，`tweak` → `/specdrive-tweak`，`full` → `/specdrive-build`
6. `phase: design` 或有 change 但无 Design Doc → `/specdrive-design`
7. `phase: open` 或有活跃 change 但 `.specdrive.yaml` 缺失 → `/specdrive-open`
8. 无活跃 change → `/specdrive-open`

如果元数据与文件状态冲突，以文件状态为准，修正 `.specdrive.yaml` 后继续。
...
| `.specdrive.yaml` 格式异常或缺失 | 以文件状态为准，用 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set` 修正后继续 |
| 构建/测试失败 | 返回 build 阶段修复，不进入 verify |
| change 目录结构不完整 | 按 `specdrive-open` 产物要求补齐 |

### 阶段衔接

<IMPORTANT>
单次 `/specdrive` 调用从检测到的阶段开始，退出条件满足后进入下一阶段。
...
**阶段推进与自动衔接的区分**：每个子 skill 退出前都会运行阶段守卫 `--apply` 推进 `.specdrive.yaml` 的 `phase` 字段——这一步**始终发生**，与 `auto_transition` 无关。之后子 skill 运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" next <name>` 解析下一步：`auto_transition` 不为 `false` 时输出 `NEXT: auto`（自动调用下一 skill），为 `false` 时输出 `NEXT: manual`（不调用下一 skill，提示用户手动运行）。因此 `auto_transition` **只控制是否自动调用下一个 skill，不影响 phase 推进**。无论 `auto_transition` 取何值，下方的用户决策点都必须阻塞等待。

**决策点是阻塞点**：只要到达下列任一节点，当前 `/specdrive` 调用必须停住，**使用当前平台可用的用户输入/确认机制等待用户选择**。若当前平台没有结构化提问工具，则必须在对话中提出明确选项并停止流程，等待用户回复后才能继续。用户明确选择后才能写入对应状态字段、执行对应操作，随后再继续自动流转。
...
| 命令 | 阶段 | 归属 | 产物 |
|------|------|------|------|
| `/specdrive-open` | 1. 开启 | OpenSpec | proposal.md、design.md、tasks.md |
| `/specdrive-design` | 2. 深度设计 | Superpowers | Design Doc、delta spec |
| `/specdrive-build` | 3. 计划与构建 | Superpowers | 实施计划、代码提交 |
| `/specdrive-verify` | 4. 验证与收尾 | Both | 验证报告、分支处理 |
| `/specdrive-archive` | 5. 归档 | OpenSpec | delta→main spec 同步、design doc 标注、归档 |
| `/specdrive-hotfix` | 预设路径 | Both | 快速修复（跳过 brainstorming） |
| `/specdrive-tweak` | 预设路径 | Both | 小改动（跳过 brainstorming 和完整 plan） |
...
/specdrive-hotfix（预设路径，跳过 brainstorming）
  open ──→ build ──→ verify ──→ archive
    ↑ 如触发升级条件 → 阻塞确认升级 → 补充 Design Doc → 回到完整流程

/specdrive-tweak（预设路径，跳过 brainstorming 和完整 plan）
...
| `isolation` | `branch` 或 `worktree`，工作区隔离方式。full 初始化可为 `null`，但只允许持续到 `/specdrive-build` Step 3 前；hotfix/tweak 默认 `branch` |
| `verify_mode` | `light` 或 `full`，可为空 |
| `auto_transition` | `true` 或 `false`。只控制阶段守卫推进 phase 后是否自动调用下一个 skill；`false` 时由 `specdrive-state next` 输出 `manual`，暂停下一 skill 调用，但不阻止 phase 字段更新 |
...
- 这些约束同时存在于 `specdrive-guard.sh build --apply` 和 `specdrive-state.sh transition <name> build-complete`

### 脚本定位

SpecDrive 脚本随 skill 包分发在 `specdrive/scripts/` 下。**不硬编码路径** — 定位一次，缓存到环境变量。此块为标准样板，在每个子 skill 中独立重复以确保可独立加载；修改时必须保持所有文件同步（样板版本: `v2`，变更时更新此版本号便于定位需要同步的文件）：
...
**自动状态更新**：guard 支持 `--apply` 参数，验证通过后自动更新 `.specdrive.yaml` 状态字段：

```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> <phase> --apply
```

`--apply` 内部委托给 `specdrive-state transition`。需要直接表达状态事件时使用：
...
加载 specdrive 后，agent 应执行以上变量赋值一次，后续全程复用 `$SPECDRIVE_GUARD`、`$SPECDRIVE_STATE`、`$SPECDRIVE_HANDOFF`、`$SPECDRIVE_ARCHIVE`。
...
.specdrive/
└── config.yaml                        # SpecDrive 项目配置（context_compression 默认 off，可设 beta）
```
...
3. **交接包由脚本生成** — OpenSpec → Superpowers 的上下文必须通过 `specdrive-handoff.sh` 生成 compact 可追溯摘录（需要全文时用 `--full`），并由 guard 校验 source/hash/mode
4. **保持 tasks.md 同步** — 完成一个勾一个
5. **频繁提交** — 每个任务一次提交，message 体现设计意图
6. **先验证再确认归档** — `/specdrive-verify` 通过后进入 `/specdrive-archive`，但运行归档脚本前必须等待用户最终确认
