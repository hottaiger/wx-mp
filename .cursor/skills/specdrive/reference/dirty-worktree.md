...
规范路径：`specdrive/reference/dirty-worktree.md`

本协议由所有涉及代码修改的 specdrive 子 skill 共享。当 agent 恢复上下文或继续执行时，必须按本协议处理未提交的工作区改动。各子 skill 可在本协议基础上定义阶段特例（如 verify 阶段对实现改动的特殊处理），详见对应子 skill 文件。本文件不重复阶段特例。
...
- dirty worktree 只代表代码事实，不会自动推进 `.specdrive.yaml` 的 `phase` 或勾选 `tasks.md`；只有完成归因、验证、同步必要文档，并通过对应阶段 guard 后，才允许推进 SpecDrive 状态
