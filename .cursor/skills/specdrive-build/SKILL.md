---
name: specdrive-build
description: "SpecDrive 阶段 3：计划与构建。用 /specdrive-build 调用。制定计划并选择执行方式（subagent 或直接执行）实施。"
---

# SpecDrive 阶段 3：计划与构建（Build）
...
**幂等性**：build 阶段所有操作可安全重复执行。读取 `.specdrive.yaml` 的 `phase` 字段确认仍在 build 阶段，读取 plan 文件头的 `base-ref`，再用 `grep -n '\- \[ \]' tasks.md | head -1` 找到第一个未勾选任务继续执行。已提交的任务不得重复提交。
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> plan docs/superpowers/plans/YYYY-MM-DD-feature.md
```
...
| B | 暂停切换模型 | 记录 `build_pause: plan-ready`，本次 `/specdrive-build` 停止，用户稍后可从 `/specdrive` 或 `/specdrive-build` 恢复 |
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> build_pause null
```

用户选择暂停时：

```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> build_pause plan-ready
```
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> isolation <branch|worktree>
```

- 若用户选择 `executing-plans`：运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> subagent_dispatch null`，再运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> build_mode executing-plans`
- 若用户选择 `subagent-driven-development`：先确认当前平台存在可调用的真实后台 subagent / Task / multi-agent 调度能力；确认后先运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> subagent_dispatch confirmed`，再运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> build_mode subagent-driven-development`
...
运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> tdd_mode <tdd|direct>`

`isolation` 是脚本级硬约束。full workflow 初始化时可以为 `null`，但只允许存在到本步骤之前。若保持 `null`，`build → verify` 的 guard 和 `specdrive-state transition build-complete` 都会失败。

`subagent_dispatch` 是脚本级硬约束。`build_mode: subagent-driven-development` 离开 build 阶段前必须同时满足 `subagent_dispatch: confirmed`，否则 `specdrive-guard.sh build --apply` 和 `specdrive-state transition build-complete` 都会失败。

`tdd_mode` 是脚本级硬约束。full workflow 离开 build 阶段前 `tdd_mode` 必须已选择为 `tdd` 或 `direct`，否则 `specdrive-guard.sh build --apply` 和 `specdrive-state transition build-complete` 都会失败。

`build_mode` 默认仅 hotfix/tweak preset 使用 `direct`。full workflow 不得默认使用 `direct`。只有用户明确要求跳过计划执行技能，且你已记录显式 override 时，才允许：

```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> direct_override true
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> build_mode direct
```
...
- **branch**：根据 workflow 类型和当前日期推荐分支名，然后让用户确认或输入自定义名称。这是用户决策点——**必须使用当前平台可用的用户输入/确认机制暂停并等待用户明确确认或覆盖分支名**，不得跳过此步骤直接创建分支。
  ...
  分支命名规范：
  - 读取 `.specdrive.yaml` 的 `workflow` 字段确定前缀
...
- 如果当前平台没有真实后台 subagent / Task / multi-agent 调度能力，必须暂停并等待用户选择改用主窗口执行。用户选择改用主窗口执行后，必须先运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> build_mode executing-plans`，再按 `build_mode: executing-plans` 分支加载 Superpowers `executing-plans` 技能。用户未明确选择前，不得继续执行任务。
...
要求：
- `requesting-code-review` 技能必须在 `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> build --apply` 之前加载
...
| 大 | 全新 capability 需求 | **必须使用当前平台可用的用户输入/确认机制暂停并等待用户确认拆分**；用户确认后，通过 `/specdrive-open` 创建独立 change |
...
创建独立 change 时必须调用 `/specdrive-open`，不得直接调用 `/opsx:new`。`/specdrive-open` 会同时创建 OpenSpec 产物和 `.specdrive.yaml`，避免新 change 脱离 SpecDrive 状态机。

**用户选择必须包含**：
- 「拆分为新 change」— 通过 `/specdrive-open` 创建独立 change
...
- **每完成一个 task**：立即勾选 Superpowers plan 中的对应任务；若任务映射到 OpenSpec tasks.md，也勾选对应 OpenSpec 任务；然后提交代码，确保 `.specdrive.yaml` 和文件状态持久化。用 `grep -c '\- \[ \]' tasks.md` 检查剩余未勾选数，无需重新读取整个文件
- **上下文压缩后恢复**：先运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" check <change-name> build --recover`，脚本输出结构化恢复上下文（isolation/build_mode 状态、plan 路径、任务完成进度、恢复动作）。根据 Recovery action 决定下一步。
- **用户手动修改恢复**：按 `specdrive/reference/dirty-worktree.md` 协议处理未提交改动。该协议定义了检查步骤、归因分类和禁令。build 阶段的特殊处理：
...
- **阶段守卫**：运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> build --apply`，全部 PASS 后由守卫推进到 `phase: verify`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）
...
配置位置可为 change 的 `.specdrive.yaml`，也可为仓库根目录的 `.specdrive.yaml` / `comet.yaml` / `.comet.yml` / `comet.yml`。
...
> **术语区分**：上面的「阶段守卫推进」由 guard `--apply` 完成，更新 `.specdrive.yaml` 的 `phase` 字段——这一步**始终发生**，与 `auto_transition` 无关。本节的「自动衔接」只决定**是否自动调用下一个 skill**，由 `auto_transition` 控制。
