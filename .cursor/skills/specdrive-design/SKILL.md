---
name: specdrive-design
description: "SpecDrive 阶段 2：深度设计。用 /specdrive-design 调用。通过 brainstorming 产出 Design Doc 和 delta spec。"
---

# SpecDrive 阶段 2：深度设计（Design）
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_HANDOFF" <change-name> design --write
```

脚本会根据 change `.specdrive.yaml` 的 `context_compression` 快照生成并记录交接包。

默认 `context_compression: off` 时生成：

```text
openspec/changes/<name>/.specdrive/handoff/design-context.json
openspec/changes/<name>/.specdrive/handoff/design-context.md
```

启用 beta（项目 `.specdrive/config.yaml` 中 `context_compression: beta`，创建 change 时快照进入 `.specdrive.yaml`）时生成：

```text
openspec/changes/<name>/.specdrive/handoff/spec-context.json
openspec/changes/<name>/.specdrive/handoff/spec-context.md
```

并在 `.specdrive.yaml` 写入：
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_HANDOFF" <change-name> design --write --full
```
...
```text
Change: <change-name>
OpenSpec Context Pack: openspec/changes/<name>/.specdrive/handoff/design-context.md
Machine handoff: openspec/changes/<name>/.specdrive/handoff/design-context.json

如 context_compression: beta，则使用：
OpenSpec Context Pack: openspec/changes/<name>/.specdrive/handoff/spec-context.md
Machine handoff: openspec/changes/<name>/.specdrive/handoff/spec-context.json
...
brainstorming 产出设计方案后，**必须使用当前平台可用的用户输入/确认机制暂停并等待用户明确确认设计方案**。不得在用户确认前创建最终 Design Doc、写入 `design_doc`、运行 design guard，或进入 `/specdrive-build`。若当前平台没有结构化提问工具，则在对话中提出确认问题并停止流程，等待用户回复后才能继续。
...
```bash
mkdir -p openspec/changes/<name>/.specdrive/handoff
```

`openspec/changes/<name>/.specdrive/handoff/brainstorm-summary.md` 结构：
...
- `openspec/changes/<name>/.specdrive/handoff/brainstorm-summary.md`
- `openspec/changes/<name>/.specdrive/handoff/design-context.md`（或 beta 模式的 `spec-context.md`）
- `openspec/changes/<name>/.specdrive/handoff/design-context.json`（或 beta 模式的 `spec-context.json`）
...
```bash
# 记录 design_doc 路径
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> design_doc docs/superpowers/specs/YYYY-MM-DD-topic-design.md

# 如有 delta spec 变更，重新生成 handoff（更新 hash）
"$SPECDRIVE_BASH" "$SPECDRIVE_HANDOFF" <change-name> design --write

# 阶段守卫推进 phase 到下一阶段
"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> design --apply
```
...
- Design Doc 已创建并保存
- Design Doc frontmatter 包含 `specdrive_change`、`role: technical-design`、`canonical_spec: openspec`
- `handoff_context` 和 `handoff_hash` 已写入 `.specdrive.yaml`（由 guard 强制校验）
...
- `design_doc` 已写入 `.specdrive.yaml`
- **阶段守卫**：运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> design --apply`，全部 PASS 后由守卫推进到 `phase: build`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" check <change-name> design --recover
```
