---
name: specdrive-archive
description: "SpecDrive 阶段 5：归档。用 /specdrive-archive 调用。按 OpenSpec delta 语义合并主 spec，归档 change。"
---

# SpecDrive 阶段 5：归档（Archive）

## 前置条件

- 验证已通过（阶段 4 完成）
- 分支已处理
- `openspec/changes/<name>/.specdrive.yaml` 中 `verify_result: pass`
...
. "$SPECDRIVE_ENV"
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" check <name> archive
```
...
入口验证通过后，**必须使用当前平台可用的用户输入/确认机制暂停并等待用户确认是否立即归档**。不得在用户确认前运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_ARCHIVE" "<change-name>"`。若当前平台没有结构化提问工具，则在对话中提出同等单选问题并停止流程，等待用户回复后才能继续。
...
- 「需要调整或重新验证」— 不执行归档；运行 `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" transition <change-name> archive-reopen` 回到 `phase: verify`，再调用 `/specdrive-verify`。若验证阶段确认需要修复，再按 `/specdrive-verify` 的验证失败决策回到 `/specdrive-build`
- 「暂不归档」— 不执行归档，保留当前 `phase: archive` 状态，等待用户稍后再次调用 `/specdrive-archive`

只有用户选择「确认归档」后，才允许继续 Step 2。用户选择「需要调整或重新验证」后，必须先执行 `archive-reopen` 状态回退，不得手动编辑 `.specdrive.yaml`。
...
6. 通过 `specdrive-state transition <archive-name> archived` 更新 `archived: true`
...
- 归档后的 `.specdrive.yaml` 中 `archived: true`

归档脚本会把 `openspec/changes/<name>/` 移动到 `openspec/changes/archive/YYYY-MM-DD-<name>/`。

> **WARNING**: 归档成功后**不要再对原 change 名运行** `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> archive`，因为原活跃目录已经不存在。误调会导致 guard 报错"change directory not found"。归档完整性以脚本退出码和归档目录状态为准。

## 完成

SpecDrive 流程全部完成。如需开始新工作，调用 `/specdrive` 或 `/specdrive-open`。
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" check <change-name> archive --recover
```
